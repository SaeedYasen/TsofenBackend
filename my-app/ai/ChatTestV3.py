...
def correct_spelling(user_message):
    response = client.chat.completions.create(
        model="gpt-3.5-turbo",
        messages=[{"role": "system", "content": "قم فقط بتصحيح الأخطاء الإملائية في الجملة التالية، وأعدها دون أي تغيير في المعنى:"},
...
def chat():
    ...
    print(f"📌 الرسالة الأصلية: {original_message}")  # طباعة الرسالة الأصلية
    print(f"✅ الرسالة بعد التصحيح: {corrected_message}")  # طباعة الرسالة المصححة
    intent = detect_intent(corrected_message)
    response = client.chat.completions.create(
        model="gpt-3.5-turbo",
        messages=[{"role":"system","content":"أنت مساعد طبي رقمي. يمكنك تقديم معلومات طبية والإجابة على أسئلة حول المواعيد الطبية"},
